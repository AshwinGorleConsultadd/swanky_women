...
def generate_techpack(input_images,input_context):
    ...
    Below is an example information from a tech pack:
    Date: 30/08/2025
    Season: Fall 25
    Collection: JC & CO
    Style Name: JCC-S-FA25-DRS
     logic:
     -   FOR EXAMPLE- The product code BC-A-FA25 refers to the 'Arena' item from the Broadway Collection, released for the Fall 2025 season.
     -   use a Abbreviation Full name to designate the techpack, e.g. BC-A-FA25-3PS
     
    Style Code / Product Code Convention Format:
    [Brand/Collection Abbreviation] – [Style Abbreviation] – [Season Code][Year] 
    Example Explanation:The product code BC-A-FA25 refers to: * BC → Broadway Collection * A → Arena (style name) * FA25 → Fall 2025 Tech Pack Naming Rule Use Abbreviation + Full Name to designate each tech pack. Example for Trench Coat:JC-GR-TRC-FA25 Where: * JC = JC & Co * GR = Grandiose Collection * TRC = Trench Coat * FA25 = Fall 2025 Abbreviation Key * 3PS = 3-Piece Suit * SP = Spring * SU = Summer * FA = Fall * WI = Winter

                                                           
    Description: Women’s Asymmetrical Dress
    Brand: Swanky
    Size Range: S-XL
    Total order quantity:
    Sample Size 1st-Approval: S
    Sample Size Pre-Production:
    Sample Size Production Sample:
    """
    tech_pack_header = llm_structured(header_filler_prompt, TechPackHeader)

    past_convo.append(f"{header_filler_prompt} \n {tech_pack_header}")
    ...
    palette = extract_clothing_palette("assets/front.png")
...
You are a fashion color expert. You are given: 1. A garment image 2. A list of extracted HEX color options from the garment Task: - Select the single HEX color that best represents the MAIN garment color. - Ignore shadows, highlights, folds, and lighting variations. - Then suggest the closest Pantone TCX color as "SUGGESTED". Rules: - Do NOT guarantee Pantone accuracy. - Output valid JSON only. - Clearly mark Pantone as SUGGESTED. HEX OPTIONS: {palette}


refer below past convo for context till now:
{past_convo}""")
...
description refers to item (Name, Type, Dimensions, Material) """
...
outputformate:{MeasurementsList}"""
    ...
    color = master_json['page_2']['color_name']
